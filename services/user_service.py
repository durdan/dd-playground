...
class UserService:
    ...
    def update_user(self, user_id: int, data: Dict[str, Any]) -> User:
        ...
        updated_user = User(
            id=existing_user.id,
            name=data.get('name', existing_user.name).strip() if 'name' in data else existing_user.name,
            email=data.get('email', existing_user.email).lower().strip() if 'email' in data else existing_user.email,
